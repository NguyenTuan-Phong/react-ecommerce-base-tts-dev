...
const blog: Guide[] = [...youtubeChanelVideos, ...guides, ...intro];
...
const FearturedArticle = () => {
  ...
  return (
    <div>
      <h2 className='font-semibold text-[#fa7833] text-lg mb-2'>BÀI VIẾT MỚI NHẤT</h2>
        ...
        {latestPosts.map((post) => (
          ...
          >
            <div className='relative w-25! h-15!'>
              <img src={Img3} alt={post.title} className=' object-cover flex-shrink-0 rounded-xl' />
            </div>
            <div className='flex-1'>
              <div className='font-medium text-sm line-clamp-2' style={{ color: 'black' }}>
                {post.title}
              </div>
              <p className='font-sm text-sm' style={{ color: 'gray' }}>
                {post.date}
              </p>
            </div>
          </a>
