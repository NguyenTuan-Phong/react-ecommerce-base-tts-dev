...
const YouTubeChanel = () => {
  ...
  const sideVideos = youtubeChanelVideos
  ...
  return (
    <div>
      <section className="flex gap-4 py-[20px]">
        ...
        <p className='section-text'>/</p>
        <p className="font-bold section-text">YOUTUBE CHANEL</p>
      </section>
          ...
          <Row gutter={[32, 32]} >
            <Col xs={24} sm={24} md={12} lg={12}>
              {sideCol1.map((post) => (
                <button
                  key={post.id}
                  className="block w-full text-left group bg-transparent border-none p-0 h-[180px]!"
                ...
                >
                  ...
                  <div className="font-medium text-sm line-clamp-2 h-10" style={{ color: 'black' }}>{post.title}</div>
                </button>
              ))}
            </Col>
            <Col xs={24} sm={24} md={12} lg={12}>
