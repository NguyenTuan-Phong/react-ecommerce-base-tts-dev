import { Button, Col, DatePicker, Descriptions, Form, Input, InputNumber, Modal, Pagination, Row, Select, Spin, Table, Tag} from "antd"
...
const ManagementVoucher = () => {
    ...
    const {
        ...
        handleRemoveVoucher,
    } = useRemoveVoucher(refetchVouchers);

    const {
        isPendingGetDetailVoucher,
        ResponseGetDetailVoucher
    } = useViewDetailVoucher(isIdVoucher);
    ...
    const {
        ...
        handleCreateVoucher,
    } = useCreateVoucher(refetchVouchers);

    const {
        isPendingUpdateVoucher,
        handleUpdateVoucher
    } = useUpdateVoucher(refetchVouchers);
                ...
                <Col xs={24} sm={8} md={8} lg={8} className="text-right">
                    {dataVouchers &&
                        <Button
                            ...
                            className="p-5! bg-[#22a085]! text-[white]!"
            ...
            >
                <Form layout="vertical" className="px-4" form={form} onFinish={handleOK}>
                                        ...
                                        validator(_, val) {
                                            ...
                                            if (currentType === 0 && (val < 0 || val > 30)) {
                                                return Promise.reject(
                                                    new Error("Giá trị phần trăm phải từ 0 đến 30")
                                                );
                                            }
                                            if (val < 0) {
                                                return Promise.reject(
                    ...
                    <div className="flex justify-end gap-3 pt-6">
                        <Button onClick={() => setIsOpenModalCreateOrUpdateVoucher(false)}>
                            Hủy
                        </Button>
                        <Button
                            type="primary"
                            ...
                            (
                                ...
                                <div className="bg-white p-6 rounded-xl shadow-md w-200!">
                                    ...
                                    >
                                        ...
                                        <Descriptions.Item label="Mã khuyến mãi">{dataVoucherDetail.code}</Descriptions.Item>

                                        <Descriptions.Item label="Tên">{dataVoucherDetail.name}</Descriptions.Item>
                                        <Descriptions.Item label="Mô tả">{dataVoucherDetail.description}</Descriptions.Item>

                                        <Descriptions.Item label="Loại khuyến mãi">{dataVoucherDetail.typeText}</Descriptions.Item>
                                        <Descriptions.Item label="Giá trị">{dataVoucherDetail.value}%</Descriptions.Item>
                                        ...
                                        <Descriptions.Item label="Số lượng tổng">{dataVoucherDetail.quantity} SL</Descriptions.Item>
                                        <Descriptions.Item label="Đã sử dụng">{dataVoucherDetail.usedCount}</Descriptions.Item>

                                        <Descriptions.Item label="Giới hạn mỗi người">{dataVoucherDetail.usageLimitPerUser} lần</Descriptions.Item>
                                        <Descriptions.Item label="Còn lại">{dataVoucherDetail.remainingUsage}</Descriptions.Item>
