...
import {
  useCreateCategoryItems,
  type FormCreateCategoryItems,
} from '../hook/useCreateCategoryItems';
...
const ManagementCategory = () => {
  ...
  const [initialEditValuesItems, setInitialEditValuesItems] =
    useState<FormUpdateCategoryItems | null>(null);
  const [page, setPage] = useState(0);
  const [size, setSize] = useState(10);
  const { isPendingGetCategory, ResponseGetCategory, refetchCategory } = useGetCategory(page, size);

  const data =
    ResponseGetCategory?.data?.content.map((item) => ({
      ...item,
      key: item.id,
      isParent: true,
  ...
  const { isPendingGetCategoryDetail, ResponseCategoryDetail } = useGetCategoryDetail(isIdCategory);

  const { isPendingGetCategoryItemsDetail, ResponseGetCategoryItemsDetail } =
  ...
  const { isPendingRemoveCategory, handleRemoveCategory } = useRemoveCategory(refetchCategory);

  const { isPendingRemoveCategoryItems, handleRemoveCategoryItems } =
      ...
      render: (record: SubItem) => (
        <Space className='flex!'>
          <ButtonDelete
            ...
            danger={true}
          />
          <ButtonUpdate onclick={() => handleUpdate(record)} />
        </Space>
      ),
      width: '15%',
    },
  ...
  const columns: TableColumnsType<DataType> = [
    ...
    },
    {
      ...
      render: (_: any, __: any, index: number) => (
        <p className='font-bold'>{page * size + index + 1}</p>
      ),
    },
    {
      title: 'ID',
    ...
    <div className='relative h-full'>
      ...
      <div className='flex gap-5'>
        <Button
          ...
          className='mt-5! p-5! bg-[#fa7833]! text-[white]!'
          ...
          <div>
            {!ResponseGetCategory ? (
              <div className='text-center h-100 content-center'>
                ...
                <p className='text-gray-700'>Vui lòng tạo các loại sản phẩm cho shop của bạn.</p>
                <Button
                  icon={<PlusCircleOutlined />}
                  className='mt-5! p-5! bg-[#fa7833]! text-[white]!'
                >
              ...
              <div>
                {ResponseGetCategory.data.content.length < 1 ? (
                  <div className='text-center h-100 content-center'>
                    <b className='text-[20px]'>Shop của bạn chưa có danh sách các loại sản phẩm</b>
      ...
      <div className='mt-4 absolute bottom-1 left-1/2 transform -translate-x-1/2'>
        <Pagination
          ...
          total={ResponseGetCategory?.data.currentTotalElementsCount}
        />
      </div>
                ...
                <div>
                  {!ResponseCategoryDetail ? (
                    ...
                    <div className='text-[black]'>
                      <p className='flex'>
                        <span className='w-20 block'>ID: </span>
                        {ResponseCategoryDetail.data.id}
                      </p>
                      <p className='flex'>
                        <span className='w-20 block'>Tên loại: </span>
                        {ResponseCategoryDetail.data.name}
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div>
                  {!ResponseGetCategoryItemsDetail ? (
                    ...
                    <div className='text-[black]'>
                      <p className='flex'>
                        <span className='w-20 block'>ID: </span>
                        {ResponseGetCategoryItemsDetail.data.id}
                      </p>
                      <p className='flex'>
                        <span className='w-20 block'>Tên loại: </span>
                        {ResponseGetCategoryItemsDetail.data.name}
                      </p>
                    </div>
                  )}
                </div>
      ...
      <Modal
        title={
          <div className='text-center font-bold'>
            {idEditting
              ? `Chỉnh sửa lại tên cho loại sản phẩm ID: ${idEditting}`
              : 'Thêm loại sản phẩm'}
          </div>
        }
      ...
      >
        ...
        >
          ...
          >
            <Input placeholder='Nhập tên loại sản phẩm' className='p-3!' />
          </Form.Item>
          <Form.Item className='text-end'>
            <Button
              loading={idEditting ? isPendingUpdateCategory : isPendingCreateCategory}
              className='py-5! px-10! bg-[#fa7833]! text-[white]!'
              htmlType='submit'
            >
              {idEditting ? 'Sửa' : 'Tạo'}
            </Button>
      ...
      <Modal
        title={
          <div className='text-center font-bold'>
            {idEdittingItems
              ? `Chỉnh sửa lại loại sản phẩm con ID:${idEdittingItems}`
              : 'Thêm loại sản phẩm'}
          </div>
        }
        ...
        >
          ...
          <Form.Item
            ...
            rules={[
              {
                required: true,
                message: 'Vui chọn loại sản phẩm.',
              },
            ]}
          >
            <Select className='h-12!' placeholder='Chọn loại sản phẩm'>
              {ResponseGetCategory?.data.content.map((i) => (
          ...
          <Form.Item className='text-end'>
            <Button
              loading={
                idEdittingItems ? isPendingUpdateCategoryItems : isPendingCreateCategoryItems
              }
              className='py-5! px-10! bg-[#fa7833]! text-[white]!'
              htmlType='submit'
            >
