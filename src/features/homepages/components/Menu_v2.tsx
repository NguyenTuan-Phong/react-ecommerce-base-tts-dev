      ...
      {categoryItems.map((item) => {
        ...
        return (
          <div key={item.id} className='group relative'>
            <div className='hover:bg-[#fa7833] h-full flex gap-4 items-center hover:text-white pl-[20px] py-2'>
              <div className='text-[16px] w-[40px]'>{item?.icon}</div>

              {firstSub ? (
                <Link to={`/category/${item.id}/${firstSub.id}`} className='hover:underline'>
                  {item.name}
                </Link>
              ) : (
                <span>{item.name}</span>
